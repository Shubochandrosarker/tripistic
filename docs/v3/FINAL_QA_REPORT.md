...
> ## ✅ DEPLOYMENT UPDATE — 2026-08-12 (supersedes the verdict below)
>
> **Deploy status: FIXED — this build is LIVE in production.** The verdict below
...
>   without newlines (hPanel append bug) so sshd never saw any GHA deploy key — file
>   rebuilt and every line validated, fresh ed25519 key rotated in, chat-exposed old
>   keys removed; (2) `VPS_SSH_KEY` repo secret was unparseable — replaced via a raw
>   `ssh cat | gh secret set` pipe, no manual paste; (3) the repo's read-only deploy
>   key had been deleted on GitHub — restored, plus repo-local `core.sshCommand` so
...
about defects found — it is about **verification that this environment cannot
...
396 integration, 27 e2e — all green. Nothing regressed; the suite grew by 60
...
can forget to run. `wrangler deploy --dry-run` was **not** executed — `wrangler`
...
registry, RAG, safety and metering — and nothing that made a model call. That
...
Advisor at `/ai-platform/advisor`, sharing one turn loop — two loops would be
...
1. **Conversation messages sorted non-deterministically.** They were ordered by
   ...
   transaction and PostgreSQL's `now()` is transaction time — so both rows
...
2. **CSP would have blanked the editor preview.** `frame-src` listed only
   ...
   document's URL, so the preview needed `'self'` — and report-only mode would
...
**B1 — No model provider has ever been called.** The provider layer is unit
...
**B2 — No Cloudflare account.** Publishing, the dispatch namespace, custom
...
**B3 — No performance or accessibility measurement.** The brief names LCP < 2.5s,
...
**B4 — CSP is still report-only and still allows `'unsafe-inline'` scripts.**
Correct for today — enforcing a first-draft CSP breaks Stripe silently — but it
...
**B5 — No x402 facilitator exercised.** Verification is tested through an
...
- **AI site generation is not implemented.** `siteGenerationPrompt` and the
  ...
  match — the same treatment x402 got in the first pass. A catalogue flag
...
- The Site Builder editor has no drag handle for reordering on touch devices —
...
1. `lib/ai/chat.ts` — the turn loop, and the two ceilings on it.
2. `lib/ai/rag/retrieve.ts` — unchanged this pass, and still the place tenant
   isolation is actually enforced.
3. `lib/sites/schema.ts` — the closed section vocabulary, and the new `hidden`
   flag that drops content from the render rather than styling it away.
4. `lib/x402/verify.ts` — the ordering of the payment-row claim against the
   facilitator call.
5. `prisma/migrations/20260812060000_feature_flag_expiry` — the CHECK constraint,
